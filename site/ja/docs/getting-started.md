...
コアパッケージを導入し、モデルに `History` を渡して、アンドゥ/リドゥの対象にする生成パーシャルプロパティへ印を付けます。
...
`EditingHistory` には、非nullの `History` フィールド、プロパティ、またはプライマリーコンストラクター引数を指定できます。ジェネレーターがセッター実装と記録コードを生成するため、実行時リフレクションや動的コード生成は不要です。

続けて、まとまった編集のための [記録スコープ](recording-scopes.md) を選びます。

## `INotifyPropertyChanged` との同時利用

通知は必須ではありません。対象型が `INotifyPropertyChanged` を実装している場合、ジェネレーターは通知経路をコンパイル時に解決し、通常変更、アンドゥ、リドゥのすべてで維持します。
...
属性を指定しない場合、ジェネレーターは `RaisePropertyChanged`、`OnPropertyChanged`、`NotifyPropertyChanged` を、まず `string` 引数、次に `PropertyChangedEventArgs` 引数で探索します。対象パーシャルクラス自身の `PropertyChanged` イベントも利用でき、基底クラスで継承した `protected` メソッドも対応します。通知経路を見つけられなくても アンドゥ/リドゥは動作し、`JES005` 警告が報告されます。
