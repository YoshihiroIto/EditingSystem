...
Install the integration package when observable properties generated by
`CommunityToolkit.Mvvm` should also participate in the editing history.

```text
PM> Install-Package Jewelry.EditingSystem.CommunityToolkit.Mvvm
```

Configure the history once on the partial view model and mark each observable property that
should be undoable. Field declarations and C# 13 partial property declarations are supported.
